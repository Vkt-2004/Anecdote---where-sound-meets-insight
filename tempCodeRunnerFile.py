...
def generate_frames_night(mode):
    global camera, streaming
    last_spoken_time = time.time()
    min_speak_interval = 3  # Minimum interval between spoken messages
    ...
    while streaming:
        ...
        # Process the frame based on the selected night vision mode
        ...
        if mode == 'clahe':
            ...
            gamma = 1.5  # Adjust the gamma factor as needed
        ...
        # Announce the nearest object if enough time has passed
        if nearest_object and (time.time() - last_spoken_time > min_speak_interval):
            class_name, x1, y1, x2, y2 = nearest_object
            spoken_text = f"{class_name} detected at {int(min_distance)} centimeters"
            speak(spoken_text, LANGUAGE_MAP[SELECTED_LANGUAGE])
...
def generate_frames_edge(mode='canny'):
    ...
    while streaming:
        ...
        # Apply edge detection based on the selected mode
        if mode == 'canny':
            ...
            # Laplacian edge detection with proper handling
            laplacian = cv2.Laplacian(gray, cv2.CV_64F)
            laplacian = np.absolute(laplacian)
            edges = cv2.convertScaleAbs(laplacian)

        # Convert edges to colored format
        ...
        for result in results:
            for box in result.boxes:
                ...
                # Draw bounding box and distance text on the edge-detected frame
...
def video_feed_edge():
    global streaming
    # Get the 'mode' parameter from the query string, default to 'canny' if not provided
    mode = request.args.get('mode', default='canny', type=str)

    if not streaming:
        return jsonify({"error": "Feed not started"}), 400
    
    # Pass the mode to the generate_frames_edge function
